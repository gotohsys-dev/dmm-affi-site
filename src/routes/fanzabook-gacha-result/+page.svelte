...
{:else if products.length === 1}
	<div class="flex min-h-screen items-center justify-center">
		<Card maxWidth="max-w-lg">
			<h2 class="mb-4 text-2xl font-bold">🎯 エ〇本ガチャ結果</h2>
			...
			<a href={products[0].affiliate_url} target="_blank" rel="sponsored">
				<p class="mb-4 text-lg font-semibold transition hover:text-red-400">
					...
					{/if}
					{products[0].rarity || 'N'}({#if products[0].rank}{products[0].rank}位{:else}ランク外{/if}):{products[0].title}
				</p>
			</a>
			...
			<div class="mt-8 flex flex-col gap-4">
				...
				<a href="/fanzabook" data-sveltekit-reload class="text-blue-400 hover:underline"
					>🔁 ガチャトップに戻る</a
				>
			</div>
		</Card>
...
{:else}
	<div class="min-h-screen bg-gradient-to-br from-gray-900 to-black py-10">
		<h2 class="mb-8 text-center text-3xl font-bold text-white">🎯 エ〇本10 連ガチャ結果</h2>
					...
					<a href={p.affiliate_url} target="_blank" rel="sponsored">
						<p class="mb-3 line-clamp-2 text-sm font-semibold transition hover:text-red-400">
							{#if p.is_sale}
								<span class="font-bold text-red-500">【セール中】</span>
							{/if}
							{p.rarity || 'N'}({#if p.rank}{p.rank}位{:else}ランク外{/if}):{p.title}
						</p>
					</a>
		...
		<div class="mx-auto mt-10 flex max-w-xs flex-col gap-4 text-center">
			...
			<a href="/fanzabook" data-sveltekit-reload class="text-lg text-blue-400 hover:underline"
				>🔁 ガチャトップに戻る</a
			>
		</div>
	</div>
